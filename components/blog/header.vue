<template>
	<header>
		<h1
			class="text-xl dark:text-zinc-300 md:text-3xl lg:text-4xl m-7 font-bold text-center"
		>
			{{ title || "" }}
		</h1>
		<NuxtImg
		...
		<div class="flex w-full justify-center text-xs md:text-base my-8">
			<div
				class="md:flex text-black dark:text-zinc-300 content-center gap-8 text-xs sm:text-sm"
			>
				...
				<div class="flex items-center gap-2 flex-wrap my-5">
					...
					<template v-for="tag in tags" :key="tag">
						<span
							class="bg-gray-200 dark:bg-zinc-700 rounded-md px-2 py-1 font-semibold"
							>{{ tag }}</span
						>
					</template>
				</div>
